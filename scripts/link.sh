...
#
# Our own source lives in this repo's overlay/ so a rebase onto upstream
...
# `../..` from this script only worked when this repo happens to be
...
# run from inside this repo on a sibling layout, `--show-toplevel` returns
# this repo itself, and every symlink is created here — an
...
# to recognise a checkout (runtimeEnv.js, check-protocol.js).
